...
export class SocietyRegistrationComponent {
  ...
  save(f: any) {
    if (
      f.value.societyName != '' &&
      f.value.societyRegistrationNumber != '' &&
      f.value.societyRegistrationNumber != ''
    ) {
      console.log(f.value);
      ...
      this.service.registersociety(obj).subscribe((res) => {
        if (res) {
          this.toastr.success('Registered Successfully', 'Society');
        }
        console.log(res);
      });
    ...
    if (bearers != '' && name != '' && number != '') {
      ...
      if (this.id || this.id == 0) {
        for (let i = 0; i <= this.socitybearer.length; i++) {
          if (this.id == i) {
            this.socitybearer[i].societyMemberDesignationId =
              this.bearers.societyMemberDesignationId;
        ...
        if (
          !['1', '2', '3'].includes(this.bearers.societyMemberDesignationId)
        ) {
          ...
          if (
            !this.check.chairman &&
            this.bearers.societyMemberDesignationId === '1'
          ) {
            ...
            this.bearers.societyMemberDesignationId === '2'
          ) {
            ...
            this.bearers.societyMemberDesignationId === '3'
          ) {
            ...
            console.log(
              `${this.bearers.societyMemberDesignationId} is already Exist`
            );
  ...
  edititem(index: any) {
    ...
    for (let i = 0; i <= this.socitybearer.length; i++) {
      if (index == i) {
        console.log(
          'this.socitybearer[i].name',
          this.socitybearer[i].memberName
        );
        ...
        this.userForm.controls['number'].reset(
          this.socitybearer[i].mobileNumber
        );
      }
    }
  }
